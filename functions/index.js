...
// Initialize Third-Party Clients using Environment Variables
...
exports.stripeWebhook = onRequest({ rawBody: true }, async (req, res) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      // Creates a promo code for £10 off (1000 pence)
      ...
      // Allows your front-end confirmation page to fetch referral details
      ...
      await createBiginContact(firstName, lastName, customerEmail, referralCode);

      // --- STEP E: Send Confirmation Email via Resend ---
      await resend.emails.send({
        from: "Real Life Money <workshops@reallifemoney.co.uk>", // Update to your verified Resend domain
        to: customerEmail,
 ...
 */
async function createBiginContact(firstName, lastName, email, referralCode) {
  try {
    const accessToken = await getBiginAccessToken();
    if (!accessToken) return;

    // Bigin API endpoint (EU domain)
 ...
 */
async function getBiginAccessToken() {
